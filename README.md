...
cluster. Stolos makes it easier to manage large multi-tenant and multi-cluster
...
*   **Policy distribution**: Distribute policy definitions from a centralized
    ...
    mechanism allows Stolos to support a wide range of technologies implementing
...
*   **Hierarchical policy enforcement**: Group together namespaces and
    ...
    teams are organized. Stolos provides a set of controllers running on the
...
1.  **Namespaces**: With Stolos you have one set of namespaces that apply to all
    clusters. Stolos also introduces hierarchical namespace support giving you
    the ability to group namespaces together for common policies and to
    facilitate delegation. In Stolos, only leaf namespaces can contain
...
1.  **Hierarchical RBAC policies**: Stolos provides central management of RBAC
...
1.  **Hierarchical ResourceQuota policies**: With Stolos one can manage quota
    centrally and set quota hierarchically.
1.  **Cluster-level policies:** In addition to namespace-level policies, Stolos
...
## Stolos Concepts {#stolos-concepts}

### Glossary {#glossary}

*   **Workload cluster:** A cluster where a user runs a pod.
*   **Enrolled cluster:** A workload cluster running Stolos components.
...
Stolos with its hierarchical control allows many namespaces to be managed by the
...
With Stolos, we give admins the ability to group namespaces together and to form
...
representation of the hierarchy using cluster-level CRD(s) defined by Stolos.
Stolos can be extended to support different sources of truth (e.g. Git, GCP,
...
There's a one-time set up required to set up Stolos components described in this
...
```
$ kubectl get pods -n=stolos-system
...
```
$ kubectl get secrets -n stolos-system
NAME                                             TYPE                                  DATA      AGE
...
stolosresourcequota-controller-token-4xrw8       kubernetes.io/service-account-token   3         2d

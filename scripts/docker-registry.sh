...
# installed but this does not work, make sure you're on version 0.8.1 or later.
...
# Ensure kind v0.8.1 is installed.
...
kind &> /dev/null || (
  echo "Kind is not installed. Install v0.8.1."
  echo "https://kind.sigs.k8s.io/docs/user/quick-start/"
  exit 1
)
kind version | grep v0.8.1 || (
  echo "Using unsupported kind version. Install v0.8.1."
  echo "https://kind.sigs.k8s.io/docs/user/quick-start/"
  exit 1
)

...
# Ensure kind v0.11.0 is installed.
...
kind &> /dev/null || (
  echo "Kind is not installed. Install v0.11.0."
  echo "https://kind.sigs.k8s.io/docs/user/quick-start/"
  exit 1
)
kind version | grep v0.11.0 || (
  echo "Using unsupported kind version. Install v0.11.0."
  echo "https://kind.sigs.k8s.io/docs/user/quick-start/"
  exit 1
)

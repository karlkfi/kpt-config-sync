...
# Create a service account on the remote cluster
...
secret_name=$(kubectl get sa cluster1 -n stolos-system --context ${REMOTE_CONTEXT} -o jsonpath='{.secrets[].name}')
token=$(kubectl get secret ${secret_name} -n stolos-system --context ${REMOTE_CONTEXT} -o jsonpath='{.data.token}' | base64 -d)
ca_cert=$(kubectl get secret ${secret_name} -n stolos-system --context ${REMOTE_CONTEXT} -o jsonpath='{.data.ca\.crt}' | base64 -d)
...
remote_cluster=$(kubectl config view -o jsonpath="{.contexts[?(@.name == \"${REMOTE_CONTEXT}\")].context.cluster}")
remote_cluster_url=$(kubectl config view -o jsonpath="{.clusters[?(@.name == \"${remote_cluster}\")].cluster.server}")

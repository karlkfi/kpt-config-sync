...
function run() {
  files=(
    "$nomos/manifests/00-namespace.yaml"
...
Root Reconciler:
  -git-dir ${repodir}/rev -scope ":root" -source-format [desired source format] -policy-dir [desired policy dir]

Namespace Reconciler (bookinfo namespace)
  -git-dir ${repodir}/rev -scope "bookinfo" -policy-dir [desired policy dir]

...
@test "${FILE_NAME}: Preserve autogenerated fields when syncing services" {
  ...
  # If strategic merge is NOT being used, Nomos an Kubernetes fight over nodePort.
  # Nomos constantly deletes the value, and Kubernetes assigns a random value each time.
  ...
  nodePort1=$(kubectl get service -n test test-svc -ojson | jq -c "${selector}")
  sleep 5
  nodePort2=$(kubectl get service -n test test-svc -ojson | jq -c "${selector}")

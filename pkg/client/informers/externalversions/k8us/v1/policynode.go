/*
Copyright 2017 The Kubernetes Authors.

Licensed under the Apache License, Version 2.0 (the "License");
you may not use this file except in compliance with the License.
You may obtain a copy of the License at

    http://www.apache.org/licenses/LICENSE-2.0

Unless required by applicable law or agreed to in writing, software
distributed under the License is distributed on an "AS IS" BASIS,
WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
See the License for the specific language governing permissions and
limitations under the License.
*/

// This file was automatically generated by informer-gen

package v1

import (
	policyhierarchy_v1 "github.com/mdruskin/kubernetes-enterprise-control/pkg/api/policyhierarchy/v1"
	internalinterfaces "github.com/mdruskin/kubernetes-enterprise-control/pkg/client/informers/externalversions/internalinterfaces"
	v1 "github.com/mdruskin/kubernetes-enterprise-control/pkg/client/listers/k8us/v1"
	policyhierarchy "github.com/mdruskin/kubernetes-enterprise-control/pkg/client/policyhierarchy"
	meta_v1 "k8s.io/apimachinery/pkg/apis/meta/v1"
	runtime "k8s.io/apimachinery/pkg/runtime"
	watch "k8s.io/apimachinery/pkg/watch"
	cache "k8s.io/client-go/tools/cache"
	time "time"
)

// PolicyNodeInformer provides access to a shared informer and lister for
// PolicyNodes.
type PolicyNodeInformer interface {
	Informer() cache.SharedIndexInformer
	Lister() v1.PolicyNodeLister
}

type policyNodeInformer struct {
	factory internalinterfaces.SharedInformerFactory
}

// NewPolicyNodeInformer constructs a new informer for PolicyNode type.
// Always prefer using an informer factory to get a shared informer instead of getting an independent
// one. This reduces memory footprint and number of connections to the server.
func NewPolicyNodeInformer(client policyhierarchy.Interface, resyncPeriod time.Duration, indexers cache.Indexers) cache.SharedIndexInformer {
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options meta_v1.ListOptions) (runtime.Object, error) {
				return client.K8usV1().PolicyNodes().List(options)
			},
			WatchFunc: func(options meta_v1.ListOptions) (watch.Interface, error) {
				return client.K8usV1().PolicyNodes().Watch(options)
			},
		},
		&policyhierarchy_v1.PolicyNode{},
		resyncPeriod,
		indexers,
	)
}

func defaultPolicyNodeInformer(client policyhierarchy.Interface, resyncPeriod time.Duration) cache.SharedIndexInformer {
	return NewPolicyNodeInformer(client, resyncPeriod, cache.Indexers{cache.NamespaceIndex: cache.MetaNamespaceIndexFunc})
}

func (f *policyNodeInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&policyhierarchy_v1.PolicyNode{}, defaultPolicyNodeInformer)
}

func (f *policyNodeInformer) Lister() v1.PolicyNodeLister {
	return v1.NewPolicyNodeLister(f.Informer().GetIndexer())
}

...
// Run is a common test run functions.  Use Test* methods to set test run specific
// flags, but share this.
...
func TestParser_NewFormat(t *testing.T) {
	tests := []parserTestCase{
		{
			testFiles: fileContentMap{"system/nomos.yaml": aNomosConfig},
			root:      "foo",
		},
		// TODO(filmil): Add components tests for per-cluster addressability.
	}
	*newRepoFormat = true

...
func Update(ctx context.Context, client *syncerclient.Client, current, desired namespaceconfig.AllConfigs) status.MultiError {
	var errs status.MultiError
	impl := &differ{client: client, errs: errs}
	impl.updateNamespaceConfigs(ctx, current, desired)
	impl.updateClusterConfigs(ctx, current, desired)
...
func (d *differ) updateNamespaceConfigs(ctx context.Context, current, desired namespaceconfig.AllConfigs) {
	...
	for name := range desired.NamespaceConfigs {
		...
		if actual, found := current.NamespaceConfigs[name]; found {
			if !namespaceconfig.NamespaceConfigsEqual(&intent, &actual) {
...
func (d *differ) updateClusterConfigs(ctx context.Context, current, desired namespaceconfig.AllConfigs) {
	d.updateClusterConfig(ctx, current.ClusterConfig, desired.ClusterConfig)
	d.updateClusterConfig(ctx, current.CRDClusterConfig, desired.CRDClusterConfig)
}

func (d *differ) updateClusterConfig(ctx context.Context, current, desired *v1.ClusterConfig) {
	...
	} else if !clusterconfig.ClusterConfigsEqual(desired, current) {
...
func SyncsInFirstOnly(first, second map[string]v1.Sync) []string {
	var inFirstOnly []string
	for _, s := range first {
		if _, exists := second[s.Name]; !exists {
			inFirstOnly = append(inFirstOnly, s.Name)
		}
	}

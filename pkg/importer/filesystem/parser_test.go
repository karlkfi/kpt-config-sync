...
func TestParserVetErrors(t *testing.T) {
	test := parsertest.VetTest(
		...
		parsertest.Failures("ResourceQuota with scope and hierarchical quota",
			...
			fake.HierarchyConfig(fake.HierarchyConfigResource(v1.HierarchyModeHierarchicalQuota, kinds.ResourceQuota().GroupVersion(), kinds.ResourceQuota().Kind)),
			fake.Namespace("namespaces/bar"),
			fake.FileObject(scopedResourceQuota(), "namespaces/bar/rq.yaml"),
		),
		...
		parsertest.Failure("Abstract Namespace dir with uninheritable Rolebinding",
			...
			fake.HierarchyConfig(fake.HierarchyConfigResource(v1.HierarchyModeNone, kinds.RoleBinding().GroupVersion(), kinds.RoleBinding().Kind)),
			fake.RoleBindingAtPath("namespaces/rb.yaml"),
			fake.Namespace("namespaces/bar"),
		),
		...
		parsertest.Success("Multiple resources with HierarchyConfigs",
			nil,
			fake.HierarchyConfig(fake.HierarchyConfigResource(v1.HierarchyModeInherit, kinds.ResourceQuota().GroupVersion(), kinds.ResourceQuota().Kind)),
			fake.HierarchyConfig(fake.HierarchyConfigResource(v1.HierarchyModeInherit, kinds.Role().GroupVersion(), kinds.Role().Kind)),
		),
		...
		parsertest.Success("Namespaces dir with ResourceQuota and hierarchical quota inheritance",
			...
			fake.HierarchyConfigAtPath("system/rq.yaml", core.Name("resourcequota"), fake.HierarchyConfigResource(v1.HierarchyModeHierarchicalQuota, kinds.ResourceQuota().GroupVersion(), kinds.ResourceQuota().Kind)),
		...
		parsertest.Failure("Empty string name is an error",
			metadata.MissingObjectNameErrorCode,
		...
		parsertest.Failures("Repo outside system/ is an error",
			[]string{
				...
				syntax.UnsupportedObjectErrorCode,
			},
		...
		parsertest.Failures("HierarchyConfig contains a CRD",
			...
			fake.HierarchyConfig(fake.HierarchyConfigResource(v1.HierarchyModeInherit, kinds.NamespaceConfig().GroupVersion(), kinds.NamespaceConfig().Kind)),
		),
		...
		parsertest.Failures("HierarchyConfig contains a Sync",
			[]string{
				hierarchyconfig.UnsupportedResourceInHierarchyConfigErrorCode,
				hierarchyconfig.ClusterScopedResourceInHierarchyConfigErrorCode},
			fake.HierarchyConfig(fake.HierarchyConfigResource(v1.HierarchyModeInherit, kinds.Sync().GroupVersion(), kinds.Sync().Kind)),
		),
		parsertest.Failure("Invalid name for HierarchyConfig",
			metadata.InvalidMetadataNameErrorCode,
			fake.HierarchyConfig(core.Name("RBAC")),
		),
...
func TestParseClusterSelector(t *testing.T) {
	...
	test := parsertest.VetTest(
		...
		parsertest.Success("Abstract Namespace resouce selected",
			...
			cluster(prodCluster, prodLabel),
			cluster(devCluster, devLabel),
			fake.HierarchyConfig(fake.HierarchyConfigResource(v1.HierarchyModeInherit, kinds.ConfigMap().GroupVersion(), kinds.ConfigMap().Kind)),
			fake.FileObject(prodSelectorObject(), "clusterregistry/cs.yaml"),

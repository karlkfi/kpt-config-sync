...
const (
	gitSyncErrorFile = "/repo/root/error.json"
)
...
func SyncError(label string) string {
	content, err := ioutil.ReadFile(gitSyncErrorFile)
	if err != nil {
		return fmt.Sprintf("Unable to load %s: %v. Please check git-sync logs for more info: kubectl logs -n config-management-system -l %s -c git-sync", gitSyncErrorFile, err, label)
	}
	if len(content) == 0 {
		return fmt.Sprintf("%s is empty. Please check git-sync logs for more info: kubectl logs -n config-management-system -l %s -c git-sync", gitSyncErrorFile, label)
	}

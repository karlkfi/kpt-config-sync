...
func (tc *directoryTreeTestcase) Run(t *testing.T) {
	...
	for _, dir := range tc.inputs {
		tg.addDir(cmpath.RelativeSlash(dir))
	}
	...
	var directoryTreeTestcases = []directoryTreeTestcase{
		{
			...
			inputs: []string{"a"},
			expect: &ast.TreeNode{
				Relative: cmpath.RelativeSlash("a"),
		...
		},
		{
			...
			expect: &ast.TreeNode{
				...
				Children: []*ast.TreeNode{
					{
						Relative: cmpath.RelativeSlash("a/b"),
						Type:     node.AbstractNamespace,
					},
				},
			},
				...
				Children: []*ast.TreeNode{
					{
						...
						Children: []*ast.TreeNode{
							{
								Relative: cmpath.RelativeSlash("a/b/c"),
								Type:     node.AbstractNamespace,
							},
						},
					},
		...
		},
		{
			...
			expect: &ast.TreeNode{
				...
				Children: []*ast.TreeNode{
					...
					{
						Relative: cmpath.RelativeSlash("a/c"),
						Type:     node.AbstractNamespace,
					},
				},
			},

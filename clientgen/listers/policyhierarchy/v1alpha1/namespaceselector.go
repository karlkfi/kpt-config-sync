...
type NamespaceSelectorLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha1.NamespaceSelector, err error)
	// Get retrieves the NamespaceSelector from the index for a given name.
	Get(name string) (*v1alpha1.NamespaceSelector, error)
...
// List lists all NamespaceSelectors in the indexer.
func (s *namespaceSelectorLister) List(selector labels.Selector) (ret []*v1alpha1.NamespaceSelector, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.NamespaceSelector))
	})
	return ret, err
}

// Get retrieves the NamespaceSelector from the index for a given name.
func (s *namespaceSelectorLister) Get(name string) (*v1alpha1.NamespaceSelector, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("namespaceselector"), name)
	}
	return obj.(*v1alpha1.NamespaceSelector), nil
}

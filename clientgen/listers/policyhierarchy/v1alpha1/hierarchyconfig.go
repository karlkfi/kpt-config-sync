...
type HierarchyConfigLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha1.HierarchyConfig, err error)
	// Get retrieves the HierarchyConfig from the index for a given name.
	Get(name string) (*v1alpha1.HierarchyConfig, error)
...
// List lists all HierarchyConfigs in the indexer.
func (s *hierarchyConfigLister) List(selector labels.Selector) (ret []*v1alpha1.HierarchyConfig, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.HierarchyConfig))
	})
	return ret, err
}

// Get retrieves the HierarchyConfig from the index for a given name.
func (s *hierarchyConfigLister) Get(name string) (*v1alpha1.HierarchyConfig, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("hierarchyconfig"), name)
	}
	return obj.(*v1alpha1.HierarchyConfig), nil
}

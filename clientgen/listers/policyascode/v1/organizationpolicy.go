...
type OrganizationPolicyLister interface {
	...
	List(selector labels.Selector) (ret []*v1.OrganizationPolicy, err error)
...
// List lists all OrganizationPolicies in the indexer.
func (s *organizationPolicyLister) List(selector labels.Selector) (ret []*v1.OrganizationPolicy, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.OrganizationPolicy))
	})
	return ret, err
}
...
type OrganizationPolicyNamespaceLister interface {
	...
	Get(name string) (*v1.OrganizationPolicy, error)
...
// List lists all OrganizationPolicies in the indexer for a given namespace.
func (s organizationPolicyNamespaceLister) List(selector labels.Selector) (ret []*v1.OrganizationPolicy, err error) {
...
// Get retrieves the OrganizationPolicy from the indexer for a given namespace and name.
func (s organizationPolicyNamespaceLister) Get(name string) (*v1.OrganizationPolicy, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("organizationpolicy"), name)
	}
	return obj.(*v1.OrganizationPolicy), nil
}
